...
namespace GymJournal.App.Services.API
{
    public class WorkoutPlanService : IWorkoutPlanService
	{
		...
		public async Task<WorkoutPlanDto> Add(WorkoutPlanDto workoutPlan)
		{
			...
			{
				var responseObject = await response.Content.ReadFromJsonAsync<AddWorkoutPlanResponse>();
		...
		public async Task<List<WorkoutPlanDto>> GetAll()
		{
			...
			if (response.IsSuccessStatusCode)
			{
				var responseObject = await response.Content.ReadFromJsonAsync<GetAllWorkoutPlanResponse>();
				return responseObject.WorkoutPlans.OrderBy(w => w.Name).ToList();
			}
			else
			{
				throw new ServerRequestException(await response.Content.ReadAsStringAsync());
		...
		public async Task<WorkoutPlanDto> GetById(Guid id)
		{
			...
			if (response.IsSuccessStatusCode)
			{
				var responseObject = await response.Content.ReadFromJsonAsync<GetByIdWorkoutPlanResponse>();
				return new WorkoutPlanDto
				{
					Id = responseObject.Id,
					Name = responseObject.Name,
		...
		public async Task<WorkoutPlanDto> Update(WorkoutPlanDto workoutPlan)
		{
			...
			if (response.IsSuccessStatusCode)
			{
				var responseObject = await response.Content.ReadFromJsonAsync<UpdateWorkoutPlanResponse>();
				return new WorkoutPlanDto
				{
					Id = responseObject.Id,
					Name = responseObject.Name,
